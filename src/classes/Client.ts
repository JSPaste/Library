...
export class Client {
	...
	public constructor(options: Partial<JSPClientOptions> = {}) {
		...
		const endpoint = this.options.api.endsWith('/')
			? this.options.api.slice(0, -1)
			: this.options.api;
	...
	public async access(key: string, options?: AccessOptions) {
		...
		return this.http.fetch<AccessedDocument>(
	...
	public async publish(data: any, options?: PublishOptions) {
		...
		const keyLengthHeader = options?.keyLength
			? { keyLength: options.keyLength }
			: undefined;

		const secretHeader = options?.secret
		...
		const lifetimeHeader = options?.lifetime
			? { lifetime: options.lifetime }
			: undefined;

		return this.http.fetch<PublishedDocument>(
	...
	public async exists(key: string) {
		if (this.options.version < APIEndpointVersion.v2) {
			throw new Error(
				'"Exists" can only be used with API version 2 or higher.'
			);
		}

		return this.http.fetch<boolean>(
			`${this.endpoint}/documents/${key}/exists`,
			{
				method: 'GET'
			}
		);
	...
	public async edit(key: string, options: EditOptions) {
		if (this.options.version < APIEndpointVersion.v2) {
			throw new Error(
				'"Edit" can only be used with API version 2 or higher.'
			);
		}
	...
	public async remove(key: string, options: RemoveOptions) {
		...
		return this.http.fetch<{ removed: boolean }>(
			`${this.endpoint}/documents/${key}`,
			{
				method: 'DELETE',
				headers: {
